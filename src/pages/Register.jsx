	...
	useEffect(() => {
		const fetchCourses = async () => {
			try {
				...
				toast.error(
					error.message || "Ocorreu um erro ao carregar a lista de cursos",
					{
						className: "bg-white",
					}
				);
	...
	const handleSubmit = async (e) => {
		...
		try {
			if (userData.password.length < 4) {
				const error = new Error("A senha deve conter no mínimo 4 caracteres.");
					...
					<div className="w-full max-w-md px-8 space-y-4 bg-white max-sm:space-y-6">
						...
						<form className="space-y-4" onSubmit={handleSubmit}>
							...
							<Input
								...
								maxLength={45}
								error={
									inputError.statusCode === 409 ? inputError.message : null
								}
								required={true}
								onChange={(e) =>
									setUserData((prev) => ({
							...
							<Input
								...
								maxLength={255}
								error={
									inputError.statusCode === 406 ? inputError.message : null
								}
								required={true}
								onChange={(e) =>
									setUserData((prev) => ({
